...
def callback_VisuoGuding_Pose(ar_initial_pose):
    ...
    global record_imu, PoseList, PoseList_rviz, PoseList_tip, PoseList_tip_rviz, tip_marker_array_rviz, ur_pose_list

    if math.isnan(ar_initial_pose.position.x) == False and record_imu == True:

        #fix the position erro
        # ar_pose_offset = [-0.06,-0.12,0.11]
        ar_pose_offset = [-0.042,-0.09,0.13]
        ...
        #publish penpose for visualization  >> the position is inverted in rviz, so manually invert back to achieve visualization
        ...
        pen_tip_position[0] = ar_pose.position.x - displacement[0]
        pen_tip_position[1] = ar_pose.position.y - displacement[1]
        pen_tip_position[2] = ar_pose.position.z + displacement[2]
        ...
        pen_marker.pose.position.x = -(pen_tip_position[0]+ar_pose.position.x)/2
        pen_marker.pose.position.y = -(pen_tip_position[1]+ar_pose.position.y)/2
        pen_marker.pose.position.z = (pen_tip_position[2]+ar_pose.position.z)/2
        ...
        print("start moving...")
        robot.movep(ur_pose, acc=0.01, vel=0.05, wait=False)  #set wait=false to for realtime tracking, wait=true will create delay
...
if __name__=='__main__': 
    try:  
        ...
        sub_pen_position = rospy.Subscriber("VisuoGuding_Pose", Pose, callback_VisuoGuding_Pose, queue_size=1)
